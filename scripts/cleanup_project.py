...
def main():
    ...
    parser.add_argument(
        "--force", "-f", action="store_true", help="Run without interactive confirmation"
    )
    ...
    if not os.path.exists("main.py") or not os.path.exists("vibeagent.py"):
        print_color(
            "Error: Please run this script from the VibeAgent project root directory.", Colors.RED
        )
        sys.exit(1)
    ...
    if targets["root_tests"]:
        count, size = move_files_to_legacy(targets["root_tests"], "tests/legacy", "Root Test Files")
    ...
    if targets["docs_files"]:
        count, size = move_files_to_legacy(
            targets["docs_files"], "docs/legacy", "Documentation Files"
        )
        stats["moved_files"] += count
        stats["moved_size"] += size
    ...
    report_path = os.path.join(
        archive_dir, f"cleanup_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    )
